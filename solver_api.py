"""
Pure API Akamai BM v2 Solver for Basic-Fit (login.basic-fit.com)
All HTTP requests go through curl_cffi with Chrome TLS impersonation.
Playwright is used ONLY as a JavaScript engine (DOM + sensor script execution).
No direct HTTP requests are made by the browser.
...
def _attempt(email, password, proxy_url):
    ...
    with sync_playwright() as p:
        ...
        _log(f"  Chrome/{real_ua.split('Chrome/')[1][:10] if 'Chrome/' in real_ua else '?'}")
        ...
        def route_handler(route):
            _route_through_cffi(route, sess, real_ua)

        page.route("**/*", route_handler)

        try:
            result = _run_flow(page, ctx, sess, real_ua, email, password)
        finally:
            browser.close()
...
def _run_flow(page, ctx, sess, ua, email, password):
    ...
    # Step 2: Mouse activity to build trust
    _log("  Building trust...")
    ...
    # Step 3: Login POST via curl_cffi
    ...
    # Activity on challenge page + wait for sec_cpt=2
    _mouse_activity(page, rounds=6)
    for _ in range(12):
    ...
    # Retry login POST directly (no need to reload the page)
